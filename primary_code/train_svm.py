...
#cval = cross_validation.KFold(len(X_train), n_folds=50, shuffle=True, random_state=30)
##cval = cross_validation.LeaveOneOut(len(X_train))
# Set up 5-fold cross-validation
kf = KFold(n_splits=50, shuffle=True, random_state=40)

# Perform cross-validation
#scores = cross_validation.cross_val_score(cv=cval, estimator=clf, X=X_train, y=y_train, scoring='accuracy')
scores = cross_val_score(estimator=clf,X=X_train,y=y_train,scoring='accuracy',cv=kf)

print('Scores: %s' % str(scores))
print('Accuracy: %0.2f (+/- %0.2f)' % (scores.mean(), 2*scores.std()))

# Gather predictions
#predictions = cross_validation.cross_val_predict(cv=cval, estimator=clf, X=X_train, y=y_train)
